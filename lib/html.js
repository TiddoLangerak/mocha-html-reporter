...
function Html(runner) {
  ...
  runner.on('pass', function(test){
    ...
    var error = utils.escape(utils.clean(test.error));
  ...
  runner.on('fail', function(test){
    ...
    var code = utils.escape(utils.clean(test.fn.toString()));
    result += util.format('%s  <dd><div class="alert alert-error">Test failed:<pre>%s</pre></div><pre class="prettyprint"><code class="lang-javascript">%s</code></pre></dd>', indent(), error, code);
  });
}
